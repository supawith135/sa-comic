...
import { GetComic,DeleteComicByID} from "../../services/https"
...
const DataComic: React.FC = () => {
  ...
  const columns: ColumnsType<ComicsInterface> = [
    ...
    },
    {
       ...
       render: (text, record, index) => (
        <img src={record.Image} className="w3-left w3-circle w3-margin-right" width="50%" />
       )
     },
    {
      title: "Title",
      ...
      render: (record) => (
        <Space direction="horizontal" >
          {/* <NavLink to="/AddProduct"> */}
            <Button  onClick={() =>  navigate(`/EditProduct/${record.ID}`)} shape="circle" icon={<EditOutlined />} size={"large"} />
          {/* </NavLink> */}
          <Button 
          type="primary" 
  ...
  const showModal = (val: ComicsInterface) => {
    setModalText(
      `คุณต้องการลบเรื่อง "${val.Title}" หรือไม่ ?`
    );
    setDeleteId(val.ID);
    setOpen(true);
  };
